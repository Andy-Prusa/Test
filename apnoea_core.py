...
class Patient:
    ...
    #
    # UNCITED, AND THIS IS THE LARGEST UNCITED LEVER IN THE MODEL.
    # Flagged 2026-09-21 by the pre-release source audit. The regression above
    # has NO AUTHOR, NO JOURNAL AND NO YEAR anywhere in this repository, and
    # `git grep 2.34` returns only the two implementation sites. It appears in
    # none of HANDOVER's three provenance tiers, so it was never triaged.
    # Two further defects in the same three lines:
    #   * the quoted regression carries an age term (0.009*age) that
    #     height_factor() below does NOT implement -- it is height-only, and
    #     returns 1.0 at age 45 and 1.0 at age 80.
    #   * airway_scenario.html tells the recipient the FRC-BMI relation is
    #     "parameterised, not fitted to source data", which contradicts
    #     "anchored to the standing predicted-FRC regression" above. One of
    #     those two sentences is wrong and it is not recorded which.
    ...
    #
    # WHAT BLOCKS THE FIX, and it is not willingness: Buist gives CC as a
    # PERCENTAGE OF TLC and THIS MODEL HAS NO TLC. Implementing it faithfully
    # means adding a predicted TLC (Quanjer/ECSC 1993, which Milic-Emili
    # uses) rather than editing three constants. Solving the 44-year
    # crossover for TLC gives about 6.7 L at 1.75 m, robust to the FRC
    # age-term question to 0.3% -- but that is what the crossover IMPLIES,
    # not a value read from anywhere, and it must not be shipped as one.
