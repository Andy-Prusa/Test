...
pass; the one failure is `Stock obstructed, 1-5 min slope` at 4.75 against a
band of 2.4-4.4. The oxygen limb barely moves, because desaturation is set by
oxygen stores rather than by lung pressure.
...
compensating parameter.** The trade-off it exposes is the same one every lung
volume lever produces, and it is now the central open question.
...
(inert gas); at those values our Stock slope is 5.29 and 7.40 against a
