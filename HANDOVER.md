...
(inert gas); at those values our Stock slope is 4.67 and 6.74 against a
measured 3.4. We currently pass only because `vq_log_sd` is set to 0.70, below
measurement. (Those were 5.02 and 7.44 before the Kelman buffer shift was
...
| finding | how |
|---|---|
| CO2 stores are NOT the fault | venous slopes agree obstructed 1.70 vs patent 1.80 |
| the excess is entirely the a-A gap | grows +1.50 mmHg/min obstructed, −0.04 patent |
...
### The anthropometry sliders run past the model's own assumptions

Found 2026-09-11 by sweeping all 13 page sliders to their ends. Regenerated by
`handover_numbers.py`. The sliders themselves are sound — every one is read by
the model, the three scalers are exact multipliers, and the 29 parameters the
page leaves to `model.js` defaults all match `Patient`. What the sweep exposed
is that the ends of the anthropometry dials leave the model's own envelope.

**FRC falls below the model's own residual volume.** `rv` is 1100 mL. FRC at
induction is 1586 at 107 kg, 1111 at 130, and **587 at 180** — below RV from
about 131 kg (BMI 42.8), the top third of the weight slider, and below 1.65 m
at the default weight. A lung cannot begin below its residual volume.

**So the pressure floor is reached before the apnoea starts.** Alveolar
pressure at t=0 is 0.0 cmH2O at 107 kg, −15.4 at 140, and **−50.0 at 180** —
exactly the `p_collapse` floor. Above ~160 kg every pressure-derived quantity,
including the Muller stroke-volume coupling, is a constant rather than a
computation.

Both trace to the BMI-FRC relation that `apnoea_core.py` already labels a
PLACEHOLDER, extrapolated far past anything it could be calibrated for.

**And two port defects that `test_parity.py` cannot reach:**

1. **The FRC floor differs** — `max(400.0, ...)` in Python against
   `Math.max(300, ...)` in `model.js`. Reachable from the page: weight 180
   with `frcScale` 0.55 gives 323 mL, which the JS honours and Python would
   floor to 400. Parity misses it because `frcScale` is pinned at 1.0 there.
2. **`k_frc_bmi` is a `Patient` field in Python and hard-coded `0.0417` in
   `model.js`.** They agree today, but `js_params()` never sends it, so a
   change in Python would silently not reach the page.

Also: at weight 180 the two implementations differ by 2.26% at the page's
`dt=0.1`, converging to 0.08% at `dt=0.01`. That is integration error in a
small stiff lung, not a port difference — but the page runs at 0.1.

### Which benchmarks are evidence, and which are not

For any publication this is the distinction that matters.

**Independent — nothing was tuned to them:**

| paper | model | measured | grade |
|---|---|---|---|
| Heard 2017 | 288 s | 296 (IQR 244–314) | clean, both arms inside |
| O'Loughlin 2020 | 99.1% at 18.7 min | no significant desaturation | clean |
| Toner 2019 | sham 402 s | 447 (IQR 405–525) | **marginal — see below** |
| Moreault 2021 | −30.4 cmH2O | −20 (5) and −31 (10) | direction only; magnitude explicitly not asserted |

**DISCLOSURE for any paper using Toner.** The model gives 402 s; the measured
IQR is 405–525. It sits just BELOW the interquartile range, and
`test_validation.py` asserts 380–525 — a band widened at the bottom, with the
note "we allow a little below". It passes a band we chose. State this rather
than let a reviewer find it.

**Fitted — calibration, not prediction, and cannot be cited as validation:**
Lane 2005, Ramkumar 2011, Altermatt 2005 and Dixon 2005 (the tilt gains are
calibrated to them); Sci Rep 2023 (`co_co2_gain` fitted to its net change);
Varat 1972 and Circulation 1963 (`hb_co_exp` fitted to those two anchors,
though it is exactly 1.0 above Hb 7 and so cannot have moved any Hb 14–15
result).

**Not measurement at all:** Laviola 2020 and ICSM jet 2026 are both the
Hardman simulator. Two comparators, not two confirmations.

**Contradicted:** Tokics 1996. The model runs below the measured V/Q spread.
...
**Kelman 1966a** (*J Appl Physiol* 1966;21:1375-6) is the one paper that would
still change something: it is the source for the O2 Bohr correction, the last
unverified piece of the blood-gas chemistry. Kelman 1968 refers to it and does
not restate it.

**Eger & Severinghaus 1961** is the other one outstanding, and it matters less
