...
(inert gas); at those values our Stock slope is 5.02 and 7.44 against a
measured 3.4. We currently pass only because `vq_log_sd` is set to 0.70, below
measurement. The mechanism is the arterial-to-alveolar CO2 gap, which carries
...
with the CURVATURE of the CO2 dissociation curve — which is the half-remembered
Douglas red-cell correction, never checked. **That is why Kelman 1967 is the
critical path and not housekeeping.**

### Established by measurement

| finding | how |
|---|---|
| CO2 stores are NOT the fault | venous slopes agree obstructed 2.01 vs patent 1.80 |
| the excess is entirely the a-A gap | grows +1.72 mmHg/min obstructed, −0.04 patent |
...
| and nearly inert on the knot | 4.35 → 4.24 across that whole range |
| Stock is not an outlier | Eger & Severinghaus give 3–5 mmHg/min; we sit inside |

### Larger problem, untested

Our PATENT 1–5 min slope is **1.70** against a classical 3–5. Nothing in the
suite tests it: `test_cardiac_output` uses a 15-minute average that the
first-minute jump dominates, so it reads 2.38 and passes.
